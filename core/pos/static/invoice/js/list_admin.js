            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function (data, type, row) {
                        ...
                        buttons += '<a target="_blank" href="' + pathname + 'print/' + row.id + '/" class="dropdown-item"><i class="fas fa-print"></i> Imprimir</a> ';
                        if (row.is_draft_invoice) {
                            buttons += '<a href="' + pathname + 'update/' + row.id + '/" class="dropdown-item"><i class="fas fa-edit"></i> Editar factura</a>';
                            buttons += '<a rel="create_electronic_invoice" class="dropdown-item"><i class="fas fa-clipboard-check"></i> Generar factura</a>';
                            ...
                            buttons += '<a rel="create_credit_note" class="dropdown-item"><i class="fas fa-minus-circle"></i> Crear devolución</a>';
                        }
                        buttons += '</div></div></div>';
                        return buttons;
                    }
...
$(function () {
    ...
    $('#data tbody')
        ...
        .on('click', 'a[rel="create_electronic_invoice"]', function () {
            ...
            var args = {
                'params': params,
                'content': '¿Estas seguro de generar la factura?',
        ...
        .on('click', 'a[rel="send_receipt_by_email"]', function () {
            ...
            var args = {
                'params': params,
                'content': '¿Estas seguro de enviar el comprobante por email?',
                'success': function (request) {
                    alert_sweetalert({
        ...
        .on('click', 'a[rel="create_credit_note"]', function () {
            ...
            var args = {
                ...
                'success': function (request) {
                    alert_sweetalert({
                        'message': 'Se ha generado correctamente la devolución',
                        'timer': 2000,
                        'callback': function () {
                            tblInvoice.ajax.reload();
                        }
